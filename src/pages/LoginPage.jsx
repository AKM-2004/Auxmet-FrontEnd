...
export default function LoginPage() {
  ...
  // Handle Google OAuth callback outcomes via query parameters
  useEffect(() => {
    ...
    } else if (markers.includes("error") || markers.includes("failed") || markers.includes("failure")) {
  ...
  const handleGoogleAuth = () => {
    // OAuth requires a full redirect so the browser can follow 302s to Google
    // and receive the callback to set auth cookies. Do not use XHR here.
    ...
    try {
      const response = await axios.post(
        ...
        },
        {
          ...
          validateStatus: () => true, // handle all responses manually
        }
      );
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setError("Redirected: Please continue with Google to proceed.");
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                {showGoogleSuggestion && (
                  <button
                    type="button"
                    onClick={handleGoogleAuth}
                    className="shrink-0 bg-white/10 text-white border border-white/20 px-3 py-1.5 rounded-md hover:bg-white/20 transition"
                  >
                    Continue with Google
                  </button>
                )}
